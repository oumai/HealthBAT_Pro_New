...
#pragma mark - 提示图片验证码
- (void)showCodeImageAlert:(UIImage *)codeImage
{
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"\n" message:nil preferredStyle:UIAlertControllerStyleAlert];
    
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(15, 10, 245, 40)];
    imageView.image = codeImage;
    
    [alertController.view addSubview:imageView];
    
    [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = @"图形验证码";
    ...
    UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        NSString *imageCode = textField.text;
        
        if (imageCode.length == 0) {
            [self showText:@"请输入图形验证码"];
            return;
        }
        
        [self codeRequest:imageCode];
...
#pragma mark - 获取图片验证码
- (void)requestGetMessageImage
{
    [HTTPTool requestWithURLString:[NSString stringWithFormat:@"/api/system/getMessageImage/%@",self.treatmentModel.phoneNumber] parameters:nil type:kGET success:^(id responseObject) {
        
        NSData *data = [[NSData alloc] initWithData:[NSData dataWithBase64String:[responseObject objectForKey:@"Data"]]];
        
        [self showCodeImageAlert:[UIImage imageWithData:data]];
...
#pragma mark - 发送短信验证码
- (void)codeRequest:(NSString *)imageCode
{
    
    [HTTPTool requestWithURLString:[NSString stringWithFormat:@"%@/%@/0/%@",@"/api/Account/SendVerifyCode",self.treatmentModel.phoneNumber,imageCode] parameters:nil type:kGET success:^(id responseObject) {
        
        BATVerifycodeCell *cell = [self.confirmTableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:3]];
        
        [Tools countdownWithTime:120 End:^{
            [cell.verifyBtn setTitle:@"获取验证码" forState:UIControlStateNormal];
            cell.verifyBtn.userInteractionEnabled = YES;
        } going:^(NSString *time) {
            [cell.verifyBtn setTitle:time forState:UIControlStateNormal];
            cell.verifyBtn.userInteractionEnabled = NO;
