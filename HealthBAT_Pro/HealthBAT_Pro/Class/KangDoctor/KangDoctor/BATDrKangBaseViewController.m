...
#import "BATDrKangBaseViewController.h"
...
@interface BATDrKangBaseViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,AVAudioPlayerDelegate,YYTextViewDelegate,IFlySpeechRecognizerDelegate,IFlySpeechSynthesizerDelegate,UIWebViewDelegate,UIGestureRecognizerDelegate,STPickerSingleDelegate,STPickerDateDelegate>
...
@implementation BATDrKangBaseViewController
...
- (void)sendMsg:(NSString *)content avatarUrl:(NSString *)avatarUrl {
    ...
    if (msgHistoryArray.count > 0) {
        ...
        for (BATDrKangMsgHistoryModel * msg in msgHistoryArray) {
            ...
            if ([msg.sender isEqualToString:@"Me"]) {
                ...
                [self sendMsg:msg.content avatarUrl:self.avatarUrl];
            }
            else {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self sendMsg:bottomKey avatarUrl:self.avatarUrl];
...
- (BOOL)textView:(YYTextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    if ([text isEqualToString:@"\n"]) {
        ...
        [self sendMsg:self.inputBar.textView.text avatarUrl:self.avatarUrl];
...
- (void) onResults:(NSArray *) results isLast:(BOOL)isLast;
{
    ...
    if (isLast){
        ...
        [self sendMsg:self.result avatarUrl:self.avatarUrl];
...
- (void)pickerDate:(STPickerDate *)pickerDate year:(NSInteger)year month:(NSInteger)month day:(NSInteger)day {
    ...
    //发送年龄
    [self sendMsg:dateString avatarUrl:self.avatarUrl];
...
- (void)pickerSingle:(STPickerSingle *)pickerSingle selectedTitle:(NSString *)selectedTitle {
    ...
    //发送性别
    [self sendMsg:selectedTitle avatarUrl:self.avatarUrl];
...
- (void)linkClickWithPara:(NSDictionary *)dic {
    ...
    if ([dic.allKeys containsObject:@"flag"]) {
        ...
        [self handleFlag:flag allPara:dic];

        //发信息，并等待回复
        if (![flag isEqualToString:@"症状列表"] && ![flag isEqualToString:@"展开更多"]) {
            [self sendMsg:flag avatarUrl:self.avatarUrl];
...
- (void)handleFlag:(NSString *)flag allPara:(NSDictionary *)allPara {
    ...
    if ([flag isEqualToString:@"快速查病"]) {
        ...
        NSString *oreillyAddress = allPara[flag];
        ...
        [myGeocoder geocodeAddressString:oreillyAddress completionHandler:^(NSArray *placemarks, NSError *error) {
            if ([placemarks count] > 0 && error == nil) {
                ...
                [self openUrlWithDestination:allPara[@"content"] withHospitalLon:firstPlacemark.location.coordinate.longitude hospitalLat:firstPlacemark.location.coordinate.latitude];
            }
...
//地图
-(void)openUrlWithDestination:(NSString *)destination withHospitalLon:(double)hospitalLon hospitalLat:(double)hospitalLat {
    ...
    NSString *baiduMapUrl = [[NSString stringWithFormat:@"baidumap://map/direction?origin={{我的位置}}&destination=latlng:%f,%f|name=目的地&mode=driving&coord_type=gcj02",hospitalLat,hospitalLon]stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    NSString *gaodeMapUrl = [[NSString stringWithFormat:@"iosamap://path?sourceApplication=applicationName&sid=BGVIS1&slat=%@&slon=%@&sname=%@&did=BGVIS2&dlat=%f&dlon=%f&dname=%@&dev=0&m=0&t=0",self.lat,self.lon,@"我的位置",hospitalLat,hospitalLon,destination]stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    NSString *aaMapUrl = [NSString stringWithFormat:@"qqmap://map/routeplan?type=drive&fromcoord=%@,%@&tocoord=%f,%f&policy=1",self.lat,self.lon,hospitalLat,hospitalLon];
    ...
    UIAlertAction *acton = [UIAlertAction actionWithTitle:@"苹果地图" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        CLLocationCoordinate2D loc = CLLocationCoordinate2DMake(hospitalLat,hospitalLon);
...
- (void)sendTextRequestWithContent:(NSString *)content isSound:(BOOL)isSound {
    ...
    [self sendTextRequestWithParameters:@{
                                          @"keyword":content,
                                          @"userDeviceId":self.userDeviceId,
                                          @"userId":self.userID,
                                          @"lat":self.lat,
                                          @"lon":self.lon,
                                          @"requestSource":self.requestSource,
                                          }
                                isSound:isSound];
}
...
- (void)layoutPages {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] bk_initWithImage:[UIImage imageNamed:@"icon_lishijilu"] style:UIBarButtonItemStylePlain handler:^(id sender) {
        ...
        [YCXMenu showMenuInView:self.view fromRect:CGRectMake(SCREEN_WIDTH - 55, 0, 60, 0) menuItems:self.items selected:^(NSInteger index, YCXMenuItem *item) {
            ...
            if (self.rightBarActionBlock) {
                self.rightBarActionBlock(item.tag);
            }
    ...
    // 全局添加手势
    UITapGestureRecognizer *webTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(webViewTap:)];
...
- (BATDrKangAskInputBar *)askInputBar {
    ...
    if (!_askInputBar) {
        ...
        [_askInputBar.nextBtn bk_whenTapped:^{
            ...
            [self sendMsg:content avatarUrl:self.avatarUrl];
...
- (BATDrKangDiseaseView *)diseaseView {
    ...
    if (!_diseaseView) {
        ...
        [_diseaseView setSelectedDisease:^(NSString *disease){
            ...
            STRONG_SELF(self);
            [self.diseaseView removeFromSuperview];
            [self sendMsg:disease avatarUrl:self.avatarUrl];
